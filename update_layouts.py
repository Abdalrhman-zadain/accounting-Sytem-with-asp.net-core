...
receipts_pattern = re.compile(
    r'<div className="grid gap-6 xl:grid-cols-\[1\.2fr_0\.8fr\]">\s*<Card className="overflow-hidden p-0">(.*?)</Card>\s*<Card className="space-y-5">\s*<div>\s*<div className="text-lg font-bold text-gray-900">{selectedReceipt\?\.reference \?\? t\("salesReceivables\.section\.receiptDetails"\)}</div>\s*<div className="text-sm text-gray-500">{selectedReceipt \? `\${selectedReceipt\.customer\.code} · \${selectedReceipt\.customer\.name}` : t\("salesReceivables\.section\.receiptDetailsEmpty"\)}</div>\s*</div>\s*\{selectedReceipt \? \(\s*<>\s*(.*?)\s*</>\s*\) : <div className="text-sm text-gray-500">{t\("salesReceivables\.section\.receiptDetailsEmpty"\)}</div>\}\s*</Card>\s*</div>',
    re.DOTALL
)
...
def receipts_repl(m):
    ...
    return (
        ...
        '            {selectedReceipt ? (\n'
        '              <div className="space-y-5">\n'
        '                <div className="text-sm text-gray-500">{selectedReceipt.customer.code} · {selectedReceipt.customer.name}</div>\n'
        '                ' + details_content + '\n'
        '              </div>\n'
        '            ) : null}\n'
        '          </Modal>'
