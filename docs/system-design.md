...
- API base URL resolution lives in `frontend/lib/config/api.ts` and should prefer `NEXT_PUBLIC_API_BASE_URL`; browser requests now rewrite localhost-configured dev URLs to the current LAN host when the app is opened via a private-network address, fall back to same-origin `/api` for non-dev hosts, and use `http://localhost:3007/api` for server-side local development
...
POS lives on top of the Phase 3 sales/inventory/accounting seams rather than creating a second sales engine.
...
- session expected cash is reduced by cash refunds recorded on POS returns
...
- fully paid standard sales invoices can be unposted for correction by authorized users: the workflow returns the invoice to draft, removes the old receipt allocation link while preserving the posted receipt transaction, reverses stock and ledger impact in one transaction, and attempts to reapply the detached receipt on repost when the edited invoice remains allocatable
