...
- Customer receipts print in **Arabic only** (RTL). Payment method labels use Arabic (`نقد`, `بطاقة`, etc.). The printed header shows the configured logo, the receipt tagline, and `إيصال بيع` / `فاتورة`; `POS_RECEIPT_COMPANY_NAME` is kept for API compatibility but is not rendered on customer receipts.
- Receipt contact branding defaults to the Karshanji business card values (phone, address, tagline) and can be overridden with backend env vars: `POS_RECEIPT_PHONE`, `POS_RECEIPT_ADDRESS`, and `POS_RECEIPT_TAGLINE`. The tagline prints under the logo; phone and address print in the footer above `شكراً لزيارتكم`.
- The customer receipt uses a **restaurant-style 80mm** layout (Kashouka-inspired): centered **64px** logo, branch/tax number when configured, date/time split, invoice number, table number for dine-in, order type, then a **4-column item table** (الصنف / السعر / الكمية / الإجمالي).
