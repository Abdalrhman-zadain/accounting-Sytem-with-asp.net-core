...
- Customer receipts print in **Arabic only** (RTL). Payment method labels use Arabic (`نقد`, `بطاقة`, etc.). The printed header shows only the configured logo (plus optional branch/tax meta when set); `POS_RECEIPT_COMPANY_NAME` and `POS_RECEIPT_TAGLINE` are kept for API compatibility but are not rendered on customer receipts.
- Receipt contact branding defaults to the Karshanji business card values (phone and address) and can be overridden with backend env vars: `POS_RECEIPT_PHONE` and `POS_RECEIPT_ADDRESS`. Phone and address print in the footer above `شكراً لزيارتكم`. Phone digits use an isolated LTR span so they stay in reading order on RTL receipts.
- The customer receipt uses a **restaurant-style 80mm** layout (Kashouka-inspired): centered **28mm** logo, branch/tax number when configured, date/time split, invoice number, table number for dine-in, order type, then a **4-column item table** (الصنف / السعر / الكمية / الإجمالي).
...
- Dine-in receipts show **طاولة** only (no waiter/cashier staff line on the customer receipt).
...
### Session roll reports (إغلاق الوردية)

The three accountant roll prints (`SESSION_ROLL_REPORT`, `INVOICE_LIST_ROLL`, `ALL_RECEIPTS_ROLL`) now use the same shared 72mm layout as customer receipts (`frontend/features/pos/pos-session-roll-print.ts` → `buildThermalReceiptDocumentHtml`):

- **Shift closing report:** section titles, aligned summary rows, bordered **payment-box** for payment-method totals, emphasized net/cash lines, and cash-difference row.
- **Invoice list roll:** one aligned money row per invoice plus an emphasized session total.
- **All receipts roll:** each sale prints like a mini customer receipt — 4-column item table, bold add-on rows, totals, and per-sale payment box.
