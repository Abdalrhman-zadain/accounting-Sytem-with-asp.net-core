...
- a POS module now exists at `/pos` with backend-backed session open/close, barcode/code add, item search, cart editing, discounts, payment capture, hold/resume/void, complete sale, receipt preview/print, controlled receipt reprint, inventory issue on completion, draft journal creation for accountant review, pending-review queue actions, accounting reversal, cashier-side POS return/refund capture, detailed tax/payment/cashier/branch/item/inventory reporting, and session reporting using `SalesInvoice.invoiceType = POS`, `PosSession`, `PosPayment`, `PosReturn`, `PosReturnLine`, and `PosReturnPayment`
