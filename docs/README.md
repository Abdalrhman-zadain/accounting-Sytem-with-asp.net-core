...
- a frontend-only POS workspace scaffold now exists at `/pos` with sidebar navigation and an initial sales-terminal screen; session control, held invoices, accountant review, returns, reports, settings, inventory execution, and accounting posting are not implemented yet
