...
- a POS module now exists at `/pos` with backend-backed session open/close, barcode/code add, item search, cart editing, discounts, payment capture, hold/resume, complete sale, receipt preview/print, controlled receipt reprint, inventory issue on completion, draft journal creation for accountant review, pending-review queue actions, accounting reversal, POS return/refund capture, tax/payment/cashier/branch reporting, and session reporting using `SalesInvoice.invoiceType = POS`, `PosSession`, `PosPayment`, `PosReturn`, `PosReturnLine`, and `PosReturnPayment`
