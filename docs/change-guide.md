...
- waiter floor plan reuses `frontend/app/(erp)/pos/tables/page.tsx` with waiter-only navigation targets (`/pos/waiter/order`); dedicated waiter ordering UI lives in `frontend/features/pos/pos-waiter-order-page.tsx` (catalog + cart, confirm-to-kitchen only — no cashier payment UI). Waiter order tracking board: `frontend/features/pos/pos-waiter-orders-page.tsx` at `/pos/waiter/orders`.
- confirm-to-kitchen: `POST /pos/sales/:id/send-to-kitchen` in `pos.controller.ts` / `PosService.sendSaleToKitchen`; draft saves no longer auto-create kitchen tickets — only explicit send (waiter confirm or cashier incremental send). Sending a draft table order to the kitchen must also promote it to `HELD` and keep the table linked to that invoice so selecting the table later reopens the full submitted order. Waiter-only users may read draft/held sales from the cashier's open restaurant session (`ensureSessionAccess` waiter-shared rule) so `/pos/waiter/order` can resume occupied tables.
...
- shared responsive layout classes for POS screens (auto-fill product/table grids, register split breakpoint, touch targets): `frontend/features/pos-shared/pos-layout-classes.ts` and restaurant re-exports in `frontend/features/pos/pos-layout-classes.ts`; reuse these instead of hard-coded `md:grid-cols-*` when adding new POS grids
- narrow register UX (iPad portrait, phones): `frontend/features/pos/pos-register-mobile-cart.tsx` + `frontend/features/pos/pos-register-layout.tsx` — full-screen product catalog, sticky bottom cart bar (item count + total), and slide-up order sheet with the full `salePanel`. From `960px` (`pos-wide`) up, catalog and order panel stay side by side; cart panel is sticky from `pos-wide`. Below `960px`, cashier catalog uses `PosProductCard variant="tablet"` for larger tap targets. Pass `mobileCartBar` from `pos-page.tsx` when extending the register shell
- waiter tablet layout: `frontend/features/pos/pos-waiter-order-page.tsx` uses `PosRegisterMainGrid` from `frontend/features/pos-shared` with `useWaiterWideLayout()` (`768px` / Tailwind `waiter-wide`). From `768px` up, catalog and cart stay side by side; below `768px` (iPad mini portrait, phones), full-screen catalog + sticky cart bar + slide-up order sheet (same pattern as cashier register). Category chips filter by inventory item group; product grid is capped at 3–4 columns via `waiterProductGridClass`. Waiter bottom tabs (`frontend/features/pos/waiter-bottom-tab-bar.tsx`, `frontend/app/(erp)/pos/waiter/layout.tsx`) link **Tables** and **Orders**; hidden on `/pos/waiter/order`. Waiter-only table floor (`isWaiterOnlyUser` in `frontend/app/(erp)/pos/tables/page.tsx`) uses a **single-row toolbar header** (title + inline stat pills), hides the floor-plan subheading and manage-table UI, and simplifies occupied cards to a single **Open order** CTA. The orders kanban (`pos-waiter-orders-page.tsx`) includes **Open order** when `tableId` is present. **Resume behavior:** after confirm-to-kitchen the sale is `HELD`; the waiter order page loads both `GET /pos/sales/drafts` and `GET /pos/sales/held` (plus optional `resume` query param / table `activeInvoiceId`) so returning to an occupied table shows sent lines as read-only and allows new additions above them when `POS_ADD_ITEM_AFTER_WAITER_CONFIRM` is granted.
- dine-in table floor plan UI: `frontend/app/(erp)/pos/tables/page.tsx` — uses `posTableFloorGridClass` (denser `md` tier for portrait tablets), `posTouchButtonClass` on floor/modal actions, and `posFinePointerHoverRevealClass` on the table gear (visible on touch tablets)
- POS routes (`/pos/*`, `/pos-market/*`) keep the floating mobile nav and full content width until `1280px` (`pos-nav-desktop`) via `frontend/components/app-shell.tsx` and `usePosNavDesktopLayout()` in `frontend/lib/hooks/use-viewport-breakpoints.ts`. Non-POS ERP routes still show the fixed sidebar from `1024px` (`nav-desktop`).
...
- **Print bill before pay** on the register requires `RST_PRINT_PRE_BILL` (not `POS_PRINT_RECEIPT`); the default waiter role does not include it
