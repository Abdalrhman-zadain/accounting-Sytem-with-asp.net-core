...
- waiter tablet layout: `frontend/features/pos/pos-waiter-order-page.tsx` and `pos-waiter-orders-page.tsx` use the Tailwind `waiter-wide` breakpoint (`800px` in `frontend/tailwind.config.ts`). From `800px` up (iPad portrait and landscape), catalog and cart stay side by side; the waiter orders board shows three columns instead of mobile tabs. Below `800px` (phones), the order page stacks menu on top (~60%) and cart below (~40%) with `PosProductCard variant="tablet"` and `posProductGridClass`. Cashier register mobile/split behavior stays on `pos-wide` (`960px`) in `frontend/features/pos-shared`.
