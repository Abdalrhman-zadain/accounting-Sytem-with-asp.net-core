...
- cart lock rule: once `waiterConfirmedAt` is set on a dine-in sale, **waiter-only** register users cannot add/edit/remove or change the order until payment; cashiers with `POS_VIEW_POS_SCREEN` may still edit the cart and push kitchen VOID/ADD slips via **Update kitchen**. Lines with `kitchenSentAt` cannot be removed/changed by waiters; cashiers may void/reduce them unless the linked kitchen item is `READY` or `SERVED`
