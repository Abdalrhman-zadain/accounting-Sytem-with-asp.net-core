...
- **Waiter role:** assign POS role `WAITER` (seed user `waiter` / `waiter123` via `backend/prisma/setup-pos-waiter.ts`). Waiter-only users use `/pos/waiter/tables`, `/pos/waiter/order`, and `/pos/waiter/orders`. The waiter ordering interface is unified with the cashier's main POS register page; at `/pos/waiter/order` waiters with elevated permissions (`POS_COMPLETE_SALE`, `POS_SELECT_PAYMENT_METHOD`) can now record payments and complete sales directly. Alternatively, they can build dine-in orders and confirm via `POST /pos/sales/:id/send-to-kitchen` (prints KOT) to advance service on the waiter board: **في انتظار → تم الاستلام → مغادرة**. `مغادرة` sets the table to `CLEANING`; cashier or waiter pays afterward. After dine-in payment, the table stays on `CLEANING` until the waiter taps **Table ready / الطاولة جاهزة** on the floor plan. The floor plan also exposes a **Needs cleaning** filter when any table is in that state. After waiter confirm (`waiterConfirmedAt`), both waiter and cashier carts are locked until payment.
