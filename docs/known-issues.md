...
- POS sell-by-weight is manual entry only: integrated scales, weight-embedded barcodes, and automatic tare handling are not implemented. The waiter POS flow (`/pos/waiter/order`) does not yet open the weight-entry modal for flagged items.
