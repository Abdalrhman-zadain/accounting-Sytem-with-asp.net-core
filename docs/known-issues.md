...
- Customer receipt amounts use clean grouped LTR formatting with a left safe inset in `frontend/features/pos-shared/thermal-receipt-layout.ts` so values such as `2.50` are not clipped on the physical left edge of 80mm rolls. The customer receipt uses a minimal photo-style layout: daily order number strip, bordered items table (`الصنف` / `السعر` / `الكمية` / `الإجمالي`), summary box (`الاجمالي` / `الخصم` / `الخدمات` / `المطلوب`), cashier/payment blocks, and the address/phone footer. Item names merge only cooking-type add-ons (`COOKING_TYPE`, `COOKING_METHOD`) as words (e.g. `فوارغ شوي`); half-head, size, rice/frikeh, yogurt, and other add-ons are not printed on the customer receipt. Line-level discounts print under the item row; invoice-level `خصم` prints only for cart-level discounts above summed line discounts.
