...
- the restaurant POS addendum now includes live table and kitchen APIs, delivery company/driver endpoints, restaurant metadata persistence across draft/hold/complete flows, register-side order-type selection (DINE_IN / TAKEAWAY / DELIVERY / PICKUP toggle in cart), dine-in table controls, waiter/driver/company selectors in the cart sidebar, table transfer/merge/split action buttons now visible in the register while an order is open, cashier close-shift cash counting, and accountant review tabs with order-type and payment-method correction.
...
- dedicated operational boards: **Kitchen** at `/pos/kitchen` (KDS kanban) and **Delivery Board** at `/pos/delivery` (kanban by delivery status, driver assign, status advance). These routes use standalone feature pages; other POS workspaces still render through `PosPage`.
...
- **Kitchen staff account:** assign POS role `KITCHEN` (seed user `kitchen` / `kitchen123` via `backend/prisma/setup-pos-kitchen.ts`). Kitchen-only users land on `/pos/kitchen` after login and see only the kitchen nav item. Cashiers also have `RST_VIEW_KITCHEN_SCREEN` after re-login. Users with only `ACCOUNTANT` role are redirected to `/dashboard` if they open kitchen/register URLs — that is expected; use a cashier or kitchen account.
- **Waiter role (confirm-to-kitchen):** assign POS role `WAITER` (seed user `waiter` / `waiter123` via `backend/prisma/setup-pos-waiter.ts`). Waiter-only users use `/pos/waiter/tables` and `/pos/waiter/order` — they build dine-in table orders and call `POST /pos/sales/:id/send-to-kitchen`; after confirm the order is locked for the waiter (`waiterConfirmedAt`). Kitchen tickets are no longer created on every draft save — only on explicit send (cashier may send additional unsent lines later). Payment and register edits remain cashier-only (`POS_COMPLETE_SALE`, `/pos/register`). v1 does not yet expose full `RST_CANCEL_KOT_ITEM` UI for sent lines.
