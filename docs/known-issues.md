...
- POS sell-by-weight now supports quick-pick preset weights (`0.25`, `0.5`, `0.75`, `1` of the base weight unit) in the add-on modal when first adding the item for both cashier and waiter product selection. After a weighted item is added to the cart, the register keeps that quantity fixed: cashier cart controls no longer re-weigh or increment/decrement KG lines, so the cashier must remove the line and add it again with the correct weight. Integrated scales, weight-embedded barcodes, automatic tare handling, and richer waiter-side decimal quantity editing are still not implemented.
...
- **Waiter-confirmed order lock:** when `waiterConfirmedAt` is set on a dine-in sale, waiter-only users cannot add/edit/remove the cart until payment. Cashiers can still edit (except kitchen `READY`/`SERVED` lines), send new items to kitchen, and tap **Update kitchen / تحديث المطبخ** after voids or qty changes. Backend `saveDraft` / hold paths enforce the same waiter vs cashier rules server-side.
