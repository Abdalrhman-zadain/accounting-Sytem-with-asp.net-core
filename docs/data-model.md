...
- when `allowFractionalQuantity` is enabled on an item, POS treats `defaultSalesPrice` as price per base weight unit (for example JOD/kg), stores the entered weight as line `quantity`, and deducts that decimal quantity from warehouse stock on completion; configuration belongs in the inventory item editor only — the POS register does not expose a weight-mode setup screen
