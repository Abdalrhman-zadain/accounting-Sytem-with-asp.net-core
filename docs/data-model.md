...
- **Basic seed** (`npm run seed` / `npx prisma db seed`): full chart of accounts and masters, empty accounting history so journal entry workflows start from scratch, and POS demo catalog. The restaurant menu demo now seeds inventory groups/categories for `الاصناق`, `روس و المقادم`, `الاطباق`, `وجبات 1`, `وجبات 2`, `الفتات`, `المقبلات`, `المشروبات`, `الاضافات`, plus `السناكات`.
...
- POS tables that have an active order or historical invoice references are retained for audit history; delete attempts are rejected before the database foreign-key constraint is reached.
...
- when `allowFractionalQuantity` is enabled on an item, POS treats `defaultSalesPrice` as price per base weight unit (for example JOD/kg), stores the chosen or entered weight as line `quantity`, and deducts that decimal quantity from warehouse stock on completion; the register add-on modal now offers quick-pick weights (`0.25`, `0.5`, `0.75`, `1` of the base weight unit) when first adding the item, and seeded menu items may also expose item-specific quick picks such as `عدد واحد = 0.125 KG` for `آبوات` / `كرشات`; on receipts and the register cart those preset picks render with traditional Arabic labels (`وقية`, `نص كيلو`, `تلات أواج`, `كيلو`, `عدد واحد`) instead of numeric KG text; fixed-price add-ons selected on a weight-sale line are charged once per portion on top of the weight-based subtotal rather than being multiplied by the chosen weight; matching sell-by-weight lines share one cart row via `portionCount` (`×2`, `×3`, …) including after kitchen send for cashiers; kitchen updates are manual (**Update kitchen**) and the register preserves `portionCount` across that sync when totals match; waiter carts still clone a new row after kitchen send; configuration remains owned by the inventory item editor rather than a separate POS weight-setup screen
