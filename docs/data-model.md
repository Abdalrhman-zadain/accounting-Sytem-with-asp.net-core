...
- cashier-side POS payment capture intentionally does not expose accounting-account selection; `PosPayment` still stores `bankCashAccountId`, but that value is resolved automatically from POS settings or the active session cash drawer rather than chosen manually in the payment modal
